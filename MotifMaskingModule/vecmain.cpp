...
// MotifMaskingVector class definition using std::vector
class MotifMaskingVector {
public:
    MotifMaskingVector(const std::vector<std::vector<std::vector<float>>>& logits, const std::vector<int>& expert_kernel_sizes, const std::vector<std::vector<int>>& attention_mask)
    : logits(logits), expert_kernel_sizes(expert_kernel_sizes), attention_mask(attention_mask) {
        ...
        motif_mask_center.resize(batch_size, std::vector<int>(seq_len, 0));
        motif_mask_all.resize(batch_size, std::vector<int>(seq_len, 0));
    ...
    void motif_masking_sort_based(int i, int left, int right) {
        // Do non-maximum suppression based on the logits with threshold = 0.0 (Non overlapping motifs)
        ...
        for (auto index : sorted_indices) {
            ...
            if (motif_mask_all[i][position] != 0) {
                continue;
            }
            ...
            // Check whether overlap with previous motifs
            auto summation = std::accumulate(motif_mask_all[i].begin() + left_boundary, motif_mask_all[i].begin() + right_boundary, 0);
            if (summation == 0) {
                motif_mask_center[i][position] = expert+1;
                std::fill(motif_mask_all[i].begin() + left_boundary,motif_mask_all[i].begin() + right_boundary, expert+1);
            }
        }
    }

    std::pair<std::vector<std::vector<int>>, std::vector<std::vector<int>>> batched_motif_masking_sorted() {
        std::vector<std::future<void>> futures;
        for (int i = 0; i < batch_size; ++i) {
            futures.push_back(std::async(std::launch::async, &MotifMaskingVector::motif_masking_sort_based, this, i, 1, lengths[i] - 1));
        }
        for (auto& future : futures) {
            future.get(); // Synchronize all threads
        }
        return {motif_mask_center, motif_mask_all};
    }

private:
    std::vector<std::vector<std::vector<float>>> logits;
    std::vector<int> expert_kernel_sizes;
    std::vector<std::vector<int>> motif_mask, motif_mask_center, motif_mask_all, attention_mask;
    std::vector<int> lengths;
    size_t batch_size, seq_len, num_experts;
};

std::pair<py::array_t<int>,py::array_t<int>> motif_masking_fn(py::array_t<float> logits_arr, py::array_t<int> expertKernelSizes_arr, py::array_t<int> attentionMask_arr)
{
        ...
        // Assume MotifMaskingVector is some class that does the actual processing
        MotifMaskingVector motifMasking(logits, expert_kernel_sizes, attention_mask);
        auto [center, all] = motifMasking.batched_motif_masking_sorted();

        // Minus 1 to match the original indexing for the returned arrays 
        for (int b = 0; b < B; ++b) {
            for (int l = 0; l < L; ++l) {
                center[b][l] -= 1;
                all[b][l] -= 1;
...
PYBIND11_MODULE(MotifMasking, m) {
    m.def("motif_masking_fn", &motif_masking_fn, "Generate motif mask");
}
